...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {vol.Required(CONF_HOST): cv.string, vol.Optional(CONF_PORT, default=8085): cv.port, vol.Optional(CONF_USERNAME, default=""): cv.username, vol.Optional(CONF_PASSWORD, default=""): cv.password}
)
...
class OpenHardwareMonitorData:
    ...
    def refresh(self):
        ...
        basic = HTTPBasicAuth({self._config.get(CONF_USERNAME)}, {self._config.get(CONF_PASSWORD)})

        try:
            response = requests.get(data_url, auth=basic, timeout=30)
            if(response.status_code == 200):
                self.data = response.json()
            elif(response.status_code == 401):
                _LOGGER.debug("ConnectionError: 401 Unauthorized, Wrong username or password")
